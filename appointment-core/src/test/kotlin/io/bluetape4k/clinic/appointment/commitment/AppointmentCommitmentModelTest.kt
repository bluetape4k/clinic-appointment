...
class AppointmentCommitmentModelTest {
    ...
    fun `고객 요청은 제안 상태로 시작하고 정확한 제안 동의 후에만 확정한다`() {
        ...
        val proposed = AppointmentCommitment(
            appointmentId = 10L,
        ...
        val consent = ConsentDecision(
            subject = ProposalConsentSubject(
                proposalId = 81L,
        ...
        val confirmed = proposed.confirm(
            proposalId = 81L,
            proposal = proposal,
            proposalHash = "proposal-hash",
            consent = consent,
    ...
    fun `다른 revision에 대한 고객 동의로 제안을 확정할 수 없다`() {
        val proposal = proposal()
        val commitment = AppointmentCommitment(
            appointmentId = 10L,
            status = AppointmentCommitmentStatus.PROPOSED,
            origin = AppointmentOrigin.PATIENT,
            confirmedProposalId = null,
        ...
        val staleConsent = ConsentDecision(
            subject = ProposalConsentSubject(
                proposalId = 81L,
    ...
    fun `상품 전환 동의는 예약 제안 동의로 사용할 수 없다`() {
        ...
        val wrongSubject = ConsentDecision(
            subject = object : ConsentSubject {
    ...
    fun `proposal hash는 같은 제안에 결정적이고 일정이 달라지면 변경된다`() {
        ...
        ProposalHasher.hash(proposal) shouldBeEqualTo ProposalHasher.hash(proposal.copy())
        (ProposalHasher.hash(proposal) == ProposalHasher.hash(proposal.copy(endsAt = proposal.endsAt.plusSeconds(60)))) shouldBeEqualTo
            false
    }
