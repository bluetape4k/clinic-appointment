...
application modules. This application consumes the shared catalog and should
stay aligned with the release train.

## Decision

Consume `bluetape4k-dependencies = "1.1.1"` through the standard shared-version
sync path. Do not add application-local overrides for artifacts that the central
catalog has already removed.

## Outcome

PR #130 aligned this repository to the 1.1.1 catalog and merged after CI passed.

## Verification

- GitHub PR #130 status checks passed before merge.
...
application dependency drift fixes in the catalog whenever they affect multiple
