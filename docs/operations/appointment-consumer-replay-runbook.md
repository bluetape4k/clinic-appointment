...
- replay 요청은 `approver`, `tenantGroupId`, `clinicId`, `fromOffset`, `toOffset`,
  `dryRun`을 모두 포함해야 한다. request id는 scope/range hash에 묶이며 다른 범위로 재사용할 수 없다.
...
4. dry-run 결과와 승인자/범위를 운영 기록으로 확인한 뒤, 같은 request id와 동일 범위로
   승인된 `dryRun=false` 요청을 실행한다. 서비스는 `DRY_RUN` audit를 `REQUESTED`로
   전환한 뒤 source를 호출한다.
...
- 같은 request id의 동일한 dry-run 재호출은 기존 audit 상태를 반환한다. 실행 중 프로세스가
  종료되어 `REQUESTED`에 남은 경우 같은 범위의 재호출로 재개할 수 있다. 이미 `EXECUTED`
  또는 `REJECTED`인 요청은 재실행하지 않는다.
