...
| bounded 안내 문구가 template/log injection 경계를 넘음 | control character 또는 HTML marker가 persisted payload/log에 존재 | DTO/domain validation, renderer text/HTML escaping, raw detail 로그 금지, max 500자 | 새 producer 중지 후 code-only template; Task 1/4 validation 재실행 |
...
| detail이 민감정보 저장소로 확산 | DB/outbox/DLQ/backup/provider log에 PHI/PII 또는 raw text가 남음 | 공통 registry의 email·전화/계좌/카드형 숫자열·민감 field marker 차단, API/event negative test, appointment lifecycle 삭제와 notification terminal retention, provider request redaction을 적용; production ACL·backup·provider 설정은 활성화 전 확인 | detail producer 중지 후 code-only mode; privacy/security lane 재실행 |
