...
- [ ] **Step 1: missing/available command 상태의 RED 테스트를 작성한다**
...
- [ ] **Step 1: report schema RED 테스트를 작성한다**
...
- [ ] **Step 3: deterministic report builder를 구현한다**
...
- [ ] **Step 4: GREEN과 forbidden-field 검사를 확인한다**
...
- [ ] **Step 3: Android job을 구현한다**

`ubuntu-latest`에서 Node 22/npm ci, Java 25, `npm run cap:sync`, Android emulator
runner를 순서대로 실행한다. APK를 install하고 `MainActivity`를 launch한 뒤
`adb shell am start -W -a android.intent.action.VIEW -d
'io.bluetape4k.clinic.appointment://open/tenant-default/calendar?view=week&date=2026-08-27'`
로 deep link를 발행한다. package/process 확인 뒤 report를 만들고 APK와 report를
upload한다. 어느 단계든 실패하면 report `failed`와 job failure를 남긴다.

- [ ] **Step 4: iOS job을 구현한다**
...
- [ ] **Step 5: workflow static contract와 YAML lint를 GREEN으로 확인한다**

Run: `npm run test:native:workflow` 및 `actionlint .github/workflows/native-webview-ci.yml`
...
- [ ] **Step 3: docs/terminology/read-back을 확인한다**
...
- [ ] **Step 1: local frontend/backend proof**
...
- [ ] **Step 4: commit, push, live read-back, workflow receipt**
