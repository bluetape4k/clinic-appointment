...
- [ ] **Step 2: static/build verification**
  ...
  Run `AppointmentOutboxPerformanceTestSupport` with fixed seed `41`, 20,000 mixed legacy/appointment backlog rows, three bounded warmup rounds, fifteen measured production-claim samples, percentile calculation, and two-relay contention. Write a raw-payload-free report to `build/reports/appointment-messaging/benchmark.json`; assert bounded claim size, non-duplicate contention IDs, and p50/p95/p99 ordering. This local H2 artifact proves the claim/index contract only. Production publish-to-ack p95/p99, catch-up, lock-wait, heap/thread/queue, serializer allocation, and PostgreSQL/MySQL/Kafka SLO thresholds remain a deployment acceptance gate and must be captured with the real rollout profile before enabling the relay. `AppointmentOutboxQueryPlanTest` emits per-dialect EXPLAIN and asserts the V22 chosen index/no full scan.
...
- **Operational coverage:** Task 2/5/6/7/8/9 include dialect-specific partial/composite indexes and EXPLAIN, atomic batch claim with due/version/order/attempt fencing, DB-clock lease budget, broker auth/readiness context proof, pause/backpressure/fairness, bounded shutdown lifecycle, real singleton Kafka integration, concrete alert/runbook ownership, and a bounded local claim benchmark. Production latency/lock/SLO thresholds are intentionally a deployment gate rather than a local unit-test claim.
