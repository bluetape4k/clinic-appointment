...
- TypeScript: `~6.0.3`
- Gradle 관리 Node: `22.22.3` (Angular 22의 Node 22 지원 하한)
- GitHub Actions frontend toolchain: Node `22.22.3`, npm `11.12.0`
...
- 대상은 `frontend/appointment-frontend/package.json`,
  ...
  `frontend/appointment-frontend/build.gradle.kts`, 그리고 Angular 22가
...
4. CI 환경 불일치: Gradle과 GitHub Actions frontend 경로를 Node 22.22.3과
   npm 11.12.0으로 고정하고, 로컬 npm 경로는 Node 26.5.0과 npm 11.17.0에서
   추가로 검증한다. PR의 정확한 head CI에서 실제 버전을 다시 확인한다.
...
- `./gradlew :frontend:appointment-frontend:build`가 Angular 22 지원 Node로
  성공한다.
