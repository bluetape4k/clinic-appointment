...
- owner는 dispatcher 인스턴스 생성 시 한 번 생성하고, request는 한 notification의
  한 permit acquire 시도마다 새로 생성한다.

### 한 notification의 admission 순서

1. 전역 expirable permit을 bounded wait로 acquire한다.
2. 병원별 expirable permit을 같은 방식으로 acquire한다.
3. 두 permit을 모두 소유한 동안에만 `worker.process`를 호출한다.
4. lease의 일정 간격마다 Redis server-time 기준으로 renew한다.
...
renew가 ownership을 잃거나 integrity/backend 결과를 신뢰할 수 없게 되면 현재
작업을 중단하고 `NOT_READY` 경계로 회수한다. 취소 예외는 삼키지 않으며, cleanup
실패는 warning log와 bounded metric으로 남긴다.
...
Failure metric은 `mode`(`local`, `redis`)와 bounded `reason`만 사용하며 tenant, clinic,
member, appointment, request identity를 tag로 사용하지 않는다.
...
- `RedisServer.Launcher.redis` 하나만 사용하고 `RedisServer.TAG == "8"`을 확인한다.
