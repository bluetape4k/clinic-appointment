...
이번 작업은 Redis 7.2/8.8 이미지 매트릭스를 도입하지 않는다. `bluetape4k-testcontainers:1.12.1`의 현재 `RedisServer.Launcher.redis`는 `RedisServer.TAG = "8"`을 사용하므로 CI가 검증하는 이미지는 `redis:8`이다.
...
- resolved `bluetape4k-testcontainers` 버전은 `1.12.1`이고 `RedisServer.TAG`의 현재 값은 `8`이다.
...
- `RedisServer.TAG`이 정확히 `8`인지 확인해 의도하지 않은 기본 이미지 변경을 즉시 실패시킨다.
...
- Issue #360의 제목과 완료 조건을 `redis:8` 단일 검증으로 정정한다.
...
| 운영 | PASS | Redis 8을 지원 기준으로 명시하고, 다른 버전과의 matrix는 별도 이슈로 분리하며 실패 시 테스트·의존성만 롤백할 수 있다. |
...
- [ ] Issue #360, lesson 문서, PR 본문이 실제 단일 Redis 8 범위와 일치한다.
