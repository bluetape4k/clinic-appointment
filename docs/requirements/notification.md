...
`same-day-hours-before`는 당일 리마인더가 발송될 예약 시작 시점까지의 최대 남은 시간이다. 예를 들어 `2`이면 병원 현지 시간 기준으로 예약 시작 전 2시간 이내인 확정 예약에만 당일 리마인더를 발송한다. 이미 시작된 예약과 2시간보다 이른 예약에는 발송하지 않는다.

당일 리마인더는 각 병원의 `timezone`으로 현재 시각과 예약 날짜를 판정한다. UTC 날짜와 병원 현지 날짜가 다른 시간대도 누락되지 않도록 인접 날짜의 후보를 조회하며, `NotificationHistory`의 성공 이력으로 중복 발송을 방지한다. 전일 리마인더의 날짜 선정 규칙은 기존 동작을 유지한다.
