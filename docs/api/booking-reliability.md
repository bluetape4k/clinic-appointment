...
route-specific matcher는 generic tenant matcher보다 먼저 적용한다. 보안 실패도 이름·전화번호를
...
bounded count, allowlist reason code, 최대 32개 `triggeringAppointmentIds`, `hasAdditionalTriggers`,
opaque `auditCursor`, expiry, `evaluationDigest`, 현재 `mode`만 포함한다. `name`, `phone`, `email`,
`rawPayload`, `staffNote` 필드는 존재하지 않는다.

## Override와 clear

두 명령 모두 `Idempotency-Key` 헤더가 필요하다. 키는 1~128자의 bounded ASCII 값이며 원문은
서버에서 hash하여 저장한다. `evaluationDigest`는 현재 decision의 lowercase SHA-256이어야 한다.
...
사유로 사용한다. actor, clinic, member, 이름, 전화번호, 자유 텍스트는 body에 넣지 않는다.
...
구성하며 actor는 bounded `actorType:actorId` 참조로만 남긴다. 현재 page는 override/clear
