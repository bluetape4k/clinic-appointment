...
class AppointmentMessagingReadinessValidatorTest {
    @Test
    fun `PostgreSQL lower case production metadata makes readiness eligible`() {
        val postgres = PostgreSQLServer.Launcher.postgres
        val schema = "readiness_issue_337_${System.nanoTime()}"
        createPostgresSchema(postgres, schema)
        ...
        try {
            ...
            dataSource.connection.use { connection ->
                connection.createStatement().use { statement ->
                    ...
                    statement.execute(
                        "CREATE INDEX idx_outbox_appointment_ready ON scheduling_outbox_events(occurred_at)",
                    )
                    statement.execute(
                        "CREATE INDEX idx_outbox_appointment_lease_recovery ON scheduling_outbox_events(lease_until)",
                    )
                }
            }
            ...
            dropPostgresSchema(postgres, schema)
        }
    }
    ...
    private fun createPostgresSchema(postgres: PostgreSQLServer, schema: String) {
    ...
    private fun dropPostgresSchema(postgres: PostgreSQLServer, schema: String) {
        DriverManager.getConnection(
            postgres.jdbcUrl,
            postgres.username ?: PostgreSQLServer.USERNAME,
            postgres.password ?: PostgreSQLServer.PASSWORD,
