...
/**
 * commitment v2 공개 API가 사용하는 닫힌 오류 registry이다.
 *
...
enum class AppointmentCommitmentApiError(
    ...
    NEW_APPOINTMENT_API_REQUIRED(
        ...
        false,
        "Use the versioned appointment commitment endpoint.",
    ),
...
/**
 * 공개 오류 code만 운반하는 v2 application 예외이다.
 *
 ...
 *
 * 모든 `/api/v2` 하위 경로를 예약 commitment로 분류하면 이후 추가되는 다른 v2 API의
 * 인증·검증 오류까지 잘못된 registry로 직렬화된다. 따라서 현재 controller가 실제로
 * 소유하는 생성·조회·mutation 경로만 닫힌 집합으로 유지한다.
 */
internal fun isAppointmentCommitmentRequestPath(path: String): Boolean =
    path == "/api/v2/appointment-requests" ||
        ...
        APPOINTMENT_COMMITMENT_ITEM_PATH.matches(path) ||
        APPOINTMENT_COMMITMENT_PROPOSAL_DECISION_PATH.matches(path)
