-- Scheduling policy foundation (H2).
--
-- This is an additive expand migration. The non-null clinic_scope_key uses 0
-- for tenant defaults and the positive clinic ID for overrides so composite
-- uniqueness has the same meaning on H2, PostgreSQL, and MySQL. Historical
-- definitions, approvals, and effective snapshots are append-only.
...
-- Rolling-safe generic outbox expansion. V9 keeps plan_id for old and new plan
-- writers, but allows future policy events to omit it. V10 NOT NULL cutover for
-- aggregate columns is intentionally deferred until all writers dual-write.
