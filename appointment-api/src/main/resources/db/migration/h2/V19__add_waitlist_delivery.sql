...
CREATE TABLE scheduling_waitlist_policy_versions (
    ...
    urgency_weight INTEGER DEFAULT 0 NOT NULL,
    recovery_weight INTEGER DEFAULT 0 NOT NULL,
    benefit_weight INTEGER DEFAULT 0 NOT NULL,
    reliability_weight INTEGER DEFAULT 0 NOT NULL,
    waiting_age_weight INTEGER DEFAULT 0 NOT NULL,
    slot_fit_weight INTEGER DEFAULT 0 NOT NULL,
    status VARCHAR(24) NOT NULL,
    effective_from TIMESTAMP NOT NULL,
    effective_until TIMESTAMP,
    canonical_policy_json CLOB NOT NULL,
    ...
    CONSTRAINT ck_waitlist_policy_version_status CHECK (REGEXP_LIKE(status, '^(DRAFT|ACTIVE|RETIRED)$')),
...
CREATE TABLE scheduling_booking_restrictions (
    ...
    CONSTRAINT ck_booking_restriction_mode CHECK (
        REGEXP_LIKE(restriction_mode, '^(REQUIRES_STAFF_APPROVAL|EXCLUDE_AUTOMATIC_OFFER)$')
    )
);
...
CREATE TABLE scheduling_waitlist_vacancy_jobs (
    ...
    CONSTRAINT ck_waitlist_vacancy_job_status CHECK (
        REGEXP_LIKE(status, '^(READY|PROCESSING|OFFERED|NO_CANDIDATE|EXPIRED|FAILED)$')
    ),
...
CREATE TABLE scheduling_waitlist_command_records (
    ...
    CONSTRAINT fk_waitlist_command_records_clinic FOREIGN KEY (clinic_id)
        REFERENCES scheduling_clinics(id) ON DELETE CASCADE,
    CONSTRAINT ck_waitlist_command_record_status CHECK (REGEXP_LIKE(status, '^(PROCESSING|SUCCEEDED|FAILED)$')),
