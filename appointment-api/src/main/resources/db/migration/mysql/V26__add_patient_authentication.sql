...
CREATE UNIQUE INDEX uq_patient_accounts_tenant_subject
    ON scheduling_patient_accounts (tenant_group_id, patient_subject);
CREATE INDEX idx_patient_accounts_tenant_active
...
CREATE UNIQUE INDEX uq_patient_login_identities_tenant_key_value
    ON scheduling_patient_login_identities (tenant_group_id, identifier_key, normalized_value);
CREATE UNIQUE INDEX uq_patient_login_identities_account_key
    ON scheduling_patient_login_identities (patient_account_id, identifier_key);
CREATE INDEX idx_patient_login_identities_account
