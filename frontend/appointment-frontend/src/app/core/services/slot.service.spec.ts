  ...
  describe('getAvailableSlots()', () => {
    it('올바른 쿼리 파라미터로 HTTP GET 요청을 보낸다', async () => {
      ...
      const req = httpMock.expectOne(r =>
        r.url === '/api/tenant-a/clinics/1/slots' &&
    ...
    it('requestedDurationMinutes 파라미터가 없을 때 쿼리에 포함되지 않는다', async () => {
      ...
      const req = httpMock.expectOne(r => r.url === '/api/tenant-a/clinics/1/slots');
      req.flush({ data: [] });
      await promise;
    ...
    it('응답 data가 null이면 빈 배열을 반환한다', async () => {
      const promise = service.getAvailableSlots(1, 2, 3, '2025-06-01');
      httpMock.expectOne(r => r.url === '/api/tenant-a/clinics/1/slots').flush({ data: null });
    ...
    it('클리닉 ID가 URL 경로에 포함된다', async () => {
      ...
      const req = httpMock.expectOne(r => r.url === '/api/tenant-a/clinics/5/slots');
      req.flush({ data: [] });
      await promise;

      expect(req.request.url).toBe('/api/tenant-a/clinics/5/slots');
    });
  });
});
